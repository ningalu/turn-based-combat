...
namespace detail {
template <typename TBattle, typename TEvent>
using EventHandlerCallback = std::function<Action<TBattle>(TEvent)>;
}

template <typename TBattle, typename TEvent>
class EventHandler;

template <typename TBattle, typename... TUserPayloads>
class EventHandler<TBattle, Event<TUserPayloads...>> {
public:
  template <typename TSpecificEvent>
  using Callback = detail::EventHandlerCallback<TBattle, TSpecificEvent>;
...
public:
  ...
  template <typename TSpecificEvent>
  [[nodiscard]] Action<TBattle> PostEvent(const TSpecificEvent &e) const {
